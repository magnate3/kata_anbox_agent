...
func (a *agentGRPC) finishCreateContainer(ctr *container, req *pb.CreateContainerRequest, config *configs.Config) (resp *gpb.Empty, err error) {
	...
	if err != nil {
        ...
        //input_dir := filepath.Join(config.Rootfs, "/vmi/qemu_pipe")
	...
	//	 return emptyResp,grpcStatus.Errorf(codes.Internal, ": %s", config.Rootfs)
        //} 
...
const sockets_path string = "/dev/openvmi/sockets"  
const input_path string = "/dev/openvmi/input"  
const max_waits int =10
...
func (a *agentGRPC) CreateContainer(ctx context.Context, req *pb.CreateContainerRequest) (resp *gpb.Empty, err error) {
	...
	i := 1;
        for ; i <= max_waits; i++	{
 		if false == PathExists(sockets_path)  || false == PathExists(input_path) {
		    time.Sleep(1)
	        }else {
		    break
		}
        ...
        if i > max_waits {
		           return emptyResp, errors.New("sockets path or input path not exist")
	...
	//log_dir := filepath.Join("/run/kata-containers/shared/containers/", req.ContainerId)
	//log_dir = filepath.Join(log_dir, "/rootfs/run/agent.log")
	//if err := SetLogOut(agentLog.Logger, "/run/kata-containers/shared/containers/agent.log");  err != nil {
       	//	return emptyResp,errors.New("set agent log out fail") 
	//}
	//storages := []*pb.Storage{
	//	{
	//	                 MountPoint: "/run/sockets",
	//			 Source:     "/run/sockets",
	//			 Fstype:     "bind",
	//			 Driver:     driverEphemeralType,
	//	},
	//		       }

	//mountList2, err := addStorages(ctx, storages, a.sandbox)
	//if err != nil {
	//	return emptyResp, err
	//}
        //mountList = append(mountList, mountList2...)
	...
	if err := a.applyNetworkSysctls(ociSpec); err != nil {
        ...
        //input_dir := filepath.Join(ociSpec.Root.Path, "/vmi/qemu_pipe")
	//if err := os.MkdirAll(input_dir, 0777); err != nil {
	//              return emptyResp,err
	//}
 	//if err := unix.Mount("/run/sockets/qemu_pipe", input_dir, "", unix.MS_REC|unix.MS_BIND, ""); err != nil {
	//	 return emptyResp,grpcStatus.Errorf(codes.Internal, ": %s", ociSpec.Root.Path)
        //} 
