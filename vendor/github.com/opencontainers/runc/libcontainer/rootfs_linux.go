...
//const (
//	        qemu_pipe= "/dev/openvmi/sockets/qemu_pipe"
//	        openvmi_bridge= "/dev/openvmi/sockets/openvmi_bridge"
//	        event0= "/dev/openvmi/input/event0"
//	        event1= "/dev/openvmi/input/event1"
//	        event2="/dev/openvmi/input/event2"
//		 
//	)
func rootPathExists(path string) (bool) {
     _, err := os.Stat(path)
     if err == nil {
	 return true
     }
...
func prepareRootfs(pipe io.ReadWriter, iConfig *initConfig) (err error) {
	...
	if setupDev {
		...
		//if err := setupDevSymlinks3(config.Rootfs); err != nil {
		//return newSystemErrorWithCause(err, "setting up /dev symlinks")
		//}
	}
	...
	// operation not being perfectly split).
 	...
 	input_dir := filepath.Join(config.Rootfs, "/dev/openvmi")
        if err := os.MkdirAll(input_dir, 0777); err != nil {
	        return err
        }
	//if err := syscall.Mount("/dev/input", input_dir, "", syscall.MS_BIND|syscall.MS_REC, ""); err != nil {
	if err := unix.Mount("/dev/openvmi", input_dir, "", unix.MS_REC|unix.MS_BIND, ""); err != nil {
			return newSystemErrorWithCause(err, "mount bind /dev/openvmi failed")
		    }
        //s := filepath.Join(config.Rootfs, "/dev/openvmi/sockets/qemu_pipe")
        //if false == rootPathExists(s){
	//     return newSystemErrorWithCausef(errors.New("path not exist"), "path %q not exist", s)
	//}
	//vmi_path:= []string{"/dev/openvmi/sockets/qemu_pipe", "/dev/openvmi/sockets/openvmi_bridge", "/dev/openvmi/input/event0", "/dev/openvmi/input/event1", "/dev/openvmi/input/event2"}
	//for _, s := range vmi_path {
 	//        s = filepath.Join(config.Rootfs, s)
	//	if err := os.Chmod(s, os.FileMode(0777)); err != nil {
	//	return newSystemErrorWithCausef(err, "chmod  %q fail", s)
	//		   }
	//}
        //if err := setupAndroidSymlinks(config.Rootfs); err != nil {
        //      return newSystemErrorWithCause(err, "setting up android symlinks")
        //}	
...
func setupDevSymlinks2(rootfs string) error {
	var links2 = [][2]string{
		{"/dev/input/mice", "/dev/input/mclink"},
	}
	for _, link := range links2 {
		var (
			src = link[0]
...
func setupDevSymlinks3(rootfs string) error {
	var links2 = [][2]string{
		{"/dev/input/mice", "/dev/input/mclink"},
	}
	for _, link := range links2 {
...
func setupAndroidSymlinks(rootfs string) error {
	var links2 = [][2]string{
		{"/dev/openvmi/input/event0", "/dev/input/event0"},
		{"/dev/openvmi/input/event1", "/dev/input/event1"},
		{"/dev/openvmi/input/event2", "/dev/input/event2"},
		{"/dev/openvmi/sockets/openvmi_bridge", "/dev/dockdroid_bridge"},
		{"/dev/openvmi/sockets/qemu_pipe", "/dev/qemu_pipe"},
	}
	for _, link := range links2 {
		var (
			src = link[0]
...
func createDeviceNode(rootfs string, node *configs.Device, bind bool) error {
	...
	//if "/dev/binder59" == node.Path{
        //     dest = filepath.Join(dest, "mkt")
        //}
