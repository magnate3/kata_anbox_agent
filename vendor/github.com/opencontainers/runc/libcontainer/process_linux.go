...
func (p *initProcess) start() error {
	defer p.messageSockPair.parent.Close()
	//input_dir := filepath.Join(p.config.Config.Rootfs, "/vmi/sockets")
	//if err := os.MkdirAll(input_dir, 0777); err != nil {
	//	return newSystemErrorWithCause(err, "mkdir rootfs/sockets/")
	//}
	//if false == PathExists("/run/sockets/qemu_pipe") {
	//                      return newSystemErrorWithCausef(nil, "mount bind /run/sockets failed %s , /run/sockets/qemu_pipe not exist", p.config.Config.Rootfs)
	//  }
	//if err := unix.Mount("/run/sockets/qemu_pipe", input_dir, "", unix.MS_REC|unix.MS_BIND, ""); err != nil {
	//                      return newSystemErrorWithCausef(err, "mount bind /run/sockets failed %s", p.config.Config.Rootfs)
	//  }
	//return newSystemErrorWithCausef(nil, "cmd.Dir %s, cmd.Args[0] %s", p.cmd.Dir, p.cmd.Args[0])
	err := p.cmd.Start()
	p.process.ops = p
	// close the write-side of the pipes (controlled by child)
	p.messageSockPair.child.Close()
